"""
Motion Detector Module — Production-Level
...
Includes gesture trail export for frontend visualization.
"""
...
from config import MOTION_COOLDOWN, GESTURE_TRAIL_LENGTH
...
class MotionDetector:
    ...
    Exports trail data for frontend visualization.
    """
    ...
    def __init__(self, buffer_size=MOTION_BUFFER_SIZE):
        ...
        # Fingertip trail for visualization
        self._fingertip_trail = deque(maxlen=GESTURE_TRAIL_LENGTH)

        logger.info(f"MotionDetector initialized | buffer={buffer_size} | cooldown={self.cooldown}s")
    ...
    def update(self, landmarks_21x3):
        ...
        # Track index fingertip for trail visualization
        index_tip = landmarks_21x3[8]  # Index fingertip
        self._fingertip_trail.append({
            'x': float(index_tip[0]),
            'y': float(index_tip[1]),
            'z': float(index_tip[2]),
            't': time.time(),
    ...
    def get_trail_points(self, max_points=None):
        """
        Get fingertip trail points for frontend visualization.

        Args:
            max_points: Maximum number of points to return

        Returns:
            List of {x, y, z, t} dicts
        """
        points = list(self._fingertip_trail)
        if max_points:
